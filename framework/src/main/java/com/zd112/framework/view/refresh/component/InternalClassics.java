...
public abstract class InternalClassics<T extends InternalClassics> extends InternalAbstract implements RefreshInternal {
    ...
    public InternalClassics(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        RelativeLayout.LayoutParams lpArrow = new RelativeLayout.LayoutParams(DimenUtils.dip2px(20), DimenUtils.dip2px(20));
        ...
        if (thisView.getPaddingTop() == 0) {
            if (thisView.getPaddingBottom() == 0) {
                thisView.setPadding(thisView.getPaddingLeft(), mPaddingTop = DimenUtils.dip2px(20), thisView.getPaddingRight(), mPaddingBottom = DimenUtils.dip2px(20));
            } else {
                thisView.setPadding(thisView.getPaddingLeft(), mPaddingTop = DimenUtils.dip2px(20), thisView.getPaddingRight(), mPaddingBottom = thisView.getPaddingBottom());
            }
        } else {
            if (thisView.getPaddingBottom() == 0) {
                thisView.setPadding(thisView.getPaddingLeft(), mPaddingTop = thisView.getPaddingTop(), thisView.getPaddingRight(), mPaddingBottom = DimenUtils.dip2px(20));
            } else {
    ...
    public T setDrawableMarginRight(float dp) {
        ...
        lpArrow.rightMargin = lpProgress.rightMargin = DimenUtils.dp2px(dp);
    ...
    public T setDrawableSize(float dp) {
        ...
        lpArrow.width = lpProgress.width = DimenUtils.dp2px(dp);
        lpArrow.height = lpProgress.height = DimenUtils.dp2px(dp);
        arrowView.setLayoutParams(lpArrow);
        progressView.setLayoutParams(lpProgress);
        return self();
    }
    ...
    public T setDrawableArrowSize(float dp) {
        final View arrowView = mArrowView;
        ViewGroup.LayoutParams lpArrow = arrowView.getLayoutParams();
        lpArrow.height = lpArrow.width = DimenUtils.dp2px(dp);
        arrowView.setLayoutParams(lpArrow);
        return self();
    }
    ...
    public T setDrawableProgressSize(float dp) {
        final View progressView = mProgressView;
        ViewGroup.LayoutParams lpProgress = progressView.getLayoutParams();
        lpProgress.height = lpProgress.width = DimenUtils.dp2px(dp);
        progressView.setLayoutParams(lpProgress);
        return self();
    }
}
