...
import com.zd112.framework.utils.MediaTypeUtils;
...
abstract class BaseHelper {
    ...
    BaseHelper(HelperInfo helperInfo) {
        ...
        this.netInfo = helperInfo.getHttpInfo();
        if (netInfo != null) {
            netInfo.setFromCache(true);
        }
    ...
    private OkHttpClient initHttpClient(HelperInfo helperInfo) {
        ...
        if (netInfo == null || netInfo.getUrl() == null)
            return clientBuilder.build();
        try {
            URI uri = new URL(netInfo.getUrl()).toURI();
...
//                        originalResponse = aopChain(chain,originalRequest,CacheControl.FORCE_CACHE,true);
     ...
     * 添加请求头参数
     */
    void addHeadsToRequest(NetInfo info, Request.Builder requestBuilder) {
    ...
    NetInfo retInfo(NetInfo info, int code) {
        return retInfo(info, code, code, null);
    }

    NetInfo retInfo(NetInfo info, int netCode, int code) {
        return retInfo(info, netCode, code, null);
    }

    NetInfo retInfo(NetInfo info, int code, String resDetail) {
     ...
     * 封装请求结果
     */
    NetInfo retInfo(NetInfo info, int netCode, int code, String resDetail) {
     ...
     * 请求结果回调
     */
    void responseCallback(NetInfo info, ProgressCallback progressCallback, int code, String requestTag) {
        //同步回调
    ...
    protected RequestBody matchContentType(NetInfo info, UploadFileInfo fileInfo) {
            ...
            if (fileInfo.getFile() != null) {
                if (TextUtils.isEmpty(filePath)) {
                    ...
                    requestBody = RequestBody.create(
                            MediaTypeUtils.fetchFileMediaType(filePath, requestEncoding),
                            fileInfo.getFile());
                }
    ...
    private RequestBody packageRequestBody(NetInfo info, MediaType contentType) {
        String value;
