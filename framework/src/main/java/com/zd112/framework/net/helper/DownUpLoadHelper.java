...
import com.zd112.framework.net.annotation.DownloadStatus;
...
import com.zd112.framework.net.progress.ProgressResponseBody;
...
class DownUpLoadHelper extends BaseHelper{
     ...
     * 开始文件下载
     */
    NetInfo downloadingFile(OkHttpHelper okHttpInfo ,Response res){
        final NetInfo info = netInfo;
        ...
        try {
            ...
            if(DownloadStatus.PAUSE.equals(fileInfo.getDownloadStatus())){
                return retInfo(info,NetInfo.Message,"暂停下载");
            }
            //下载完成
            if(DownloadStatus.DOWNLOADING.equals(fileInfo.getDownloadStatus())){
                ...
                return retInfo(info,NetInfo.SUCCESS,filePath);
            }
        }catch(SocketTimeoutException e){
            return retInfo(info,NetInfo.WriteAndReadTimeOut);
        }catch (SocketException e){
            return retInfo(info,NetInfo.ConnectionInterruption,e.getMessage());
        }catch (Exception e){
            showLog("文件下载异常："+e.getMessage());
            return retInfo(info,NetInfo.ConnectionInterruption);
        }finally {
            try {
                if(null != bis)
                    bis.close();
    ...
    private long fetchCompletedSize(DownloadFileInfo fileInfo){
        ...
        try {
            saveFileNameEncrypt = EncryptUtils.MD5StringTo32Bit(url,true);
